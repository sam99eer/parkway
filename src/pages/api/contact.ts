...
export default async function handler(
  ...
  try {
    if (req.method !== "POST") throw new ReferenceError("Method not allowed");
    ...
    if (phone?.trim()?.length < 10 || phone?.length > 20)
      throw new ReferenceError("Please enter a valid Contact Number");
    ...
    const mailData = {
      ...
      html: `<div>${req.body.message}</div><p>Sent from:
                ${fullname}</p><p>Sender's Email: ${email}</p><p>Sender's Contact Number: ${phone}</p>`,
    };
